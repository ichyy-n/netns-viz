...
const defaultState = () => {
  const ns1Id = uid(), ns2Id = uid(), bridgeId = uid();
  const veth1aId = uid(), veth1bId = uid(), veth2aId = uid(), veth2bId = uid();
  return {
    namespaces: [
      { id: ns1Id, name: "ns-host", x: 60, y: 80, color: NS_COLORS[0], isDefault: true },
      { id: ns2Id, name: "ns-client", x: 500, y: 80, color: NS_COLORS[1], isDefault: false },
    ],
    bridges: [
      { id: bridgeId, name: "br0", nsId: ns1Id, ip: "10.0.0.1/24" },
    ],
    veths: [
      { id: uid(), name: "veth-pair-1", endA: { id: veth1aId, name: "veth1a", nsId: ns1Id, ip: "10.0.0.2/24", bridge: bridgeId }, endB: { id: veth1bId, name: "veth1b", nsId: ns2Id, ip: "10.0.0.3/24", bridge: null } },
    ],
    routes: [
      { id: uid(), nsId: ns2Id, dest: "default", gateway: "10.0.0.1", iface: "veth1b" },
    ],
  };
};
...
export default function NetnsVisualizer() {
  ...
  useEffect(() => {
    if (window.electronAPI) {
      // コンテナを起動して、ip netns list を実行
      window.electronAPI.docker.start().then(result => {
        console.log('Container:', result)
        return window.electronAPI.docker.exec('ip netns list')
      }).then(result => {
        console.log('netns list:', result)
      })
    }
  }, [])
  ...
  const addNs = () => {
    ...
    setModal({
      type: "addNs",
      data: { name: `ns-${idx}`, color: NS_COLORS[idx % NS_COLORS.length] },
    });
  };
  ...
  const confirmModal = () => {
    ...
    if (type === "addNs") {
      update(s => {
        ...
        s.namespaces.push({ id: uid(), name: data.name, x: maxX + NS_W + 60, y: 80, color: data.color, isDefault: false });
      });
  ...
  const deleteVeth = (id) => update(s => { s.veths = s.veths.filter(v => v.id !== id); });
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: COLORS.bg, display: "flex", flexDirection: "column", fontFamily: "'Segoe UI', system-ui, sans-serif", color: COLORS.text, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", borderBottom: `1px solid ${COLORS.border}`, background: COLORS.surface, flexShrink: 0 }}>
        ...
        <Btn small onClick={addNs}><Icon d={Icons.plus} size={12} color="#fff" /> Namespace</Btn>
        <Btn small onClick={addBridge} color={COLORS.green}><Icon d={Icons.plus} size={12} color="#fff" /> Bridge</Btn>
        <Btn small onClick={addVeth} color={COLORS.orange}><Icon d={Icons.plus} size={12} color="#fff" /> Veth Pair</Btn>
        <Btn small onClick={addRoute} color={COLORS.purple}><Icon d={Icons.plus} size={12} color="#fff" /> Route</Btn>

        <div style={{ flex: 1 }} />

        <Btn small ghost onClick={generateCommands}><Icon d={Icons.terminal} size={12} color={COLORS.textMuted} /> コマンド生成</Btn>
        <Btn small ghost onClick={() => { setState(defaultState()); setSelected(null); }}><Icon d={Icons.x} size={12} color={COLORS.textMuted} /> リセット</Btn>
            ...
            {namespaces.map(ns => {
              ...
              return (
                <g key={ns.id} onMouseDown={e => onMouseDown(e, ns.id)} style={{ cursor: "move" }}>
                  ...
                  {ns.isDefault && (
                    <text x={ns.x + 32 + ns.name.length * 8.5 + 8} y={ns.y + NS_HEADER / 2 + 1} dominantBaseline="middle" fontSize={9} fill={ns.color} fontFamily="'JetBrains Mono', monospace" opacity={0.7}>
                      (default)
                    </text>
                  )}
                  {/* Delete btn */}
